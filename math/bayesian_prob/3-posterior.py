...
def marginal(x, n, P, Pr):
    ...
    # Calculate marginal prob by summing over all possible values of P
    marginal_prob = np.sum(joint_probability)
...
def posterior(x, n, P, Pr):
    """Using Bayes' Theorem
    to test hypothetical probabilities"""
    if not isinstance(n, int) or n <= 0:
        raise ValueError("n must be a positive integer")
    ...
    pr_probability = joint_probability / np.sum(joint_probability)

    return pr_probability
